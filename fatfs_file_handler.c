...
#if SD_LOG_EN	
	#ifndef FH_DBG
	#define FH_DBG		LOG_VBS
	#endif
	
	#ifndef FH_ERR
	#define FH_ERR		LOG_ERR
	#endif
#else
	#define FH_DBG(fmt, ...)
	#define FH_ERR(fmt, ...)
...
void FH_mkfs(StoreDisk_t* disk)
{
	...
	BYTE work[_MAX_SS];		// work size must not less than the disk sector size(must sector size is _MAX_SS)
	fresult = f_mkfs(disk->name, disk->format_opt, 0, work, sizeof work);
	if(fresult != FR_OK) {
		FH_ERR("make FS[%s] err[%d]\n", disk->name, fresult);
	}
}
...
void FH_unmount(StoreDisk_t* disk)
{
	...
	fresult = f_mount(NULL, disk->name, 1);
	if(fresult != FR_OK) {
		FH_ERR("unmount SD err[%d]\n", fresult);
	}
}
	...
	if(fresult == FR_OK) {
		for(;;) {
			...
			if(fno.fattrib & AM_DIR) {    /* It is a directory */
				...
				if(fresult != FR_OK) {
					FH_ERR("FH_remove_dir(\"%s%s\") err[%d]\n", disk->name, fno.fname, fresult);
					break;
				}
			} else {   /* It is a file. */
...
FRESULT FH_read_bin(StoreDisk_t* disk, char* file_name, uint32_t offset, uint32_t numBytesToRead, uint8_t* dest)
{
	...
	fresult = f_stat(full_name, &fno);
	if (fresult != FR_OK) {
		FH_ERR("\"%s\" does not exists\n", full_name);
	  return fresult;
	}
	
	fresult = f_open(disk->file, full_name, FA_READ);
	if(fresult != FR_OK) {
		FH_ERR("opening file \"%s\" err[%d]\n", full_name, fresult);
		return fresult;
	}
...
FRESULT FH_create_file(StoreDisk_t* disk, char* file_name)
{
	...
	fresult = f_stat(full_name, &fno);
	if(fresult == FR_OK) {
		FH_ERR("\"%s\" already exists! use Update_File\n", full_name);
	  return fresult;
	} else {
		fresult = f_open(disk->file, full_name, FA_CREATE_ALWAYS|FA_READ|FA_WRITE);
		if(fresult != FR_OK) {
			FH_ERR("create file \"%s\" err[%d]\n", full_name, fresult);
		  return fresult;
		} else {
			FH_DBG("\"%s\" created ok. write by Write_File\n", full_name);
		}
...
FRESULT FH_get_file_info(StoreDisk_t* disk, char* file_name)
{
	...
	fresult = f_stat(full_name, &fno);
	if (fresult != FR_OK) {
		FH_ERR("get file[%s] info err[%d]\n", full_name, fresult);
		return fresult;
	}
	
	FH_printf("\n****** file: %s *******\n", full_name);
	FH_printf("\tAttr: 0x%02x\n", fno.fattrib);
	FH_printf("\tSize: %d Bytes\n", (int)fno.fsize);
	FH_printf("\tDate: %04d-%02d-%02d %02d:%02d:%02d\n\n", 
											1980 + (fno.fdate >> 9), 
											(fno.fdate >> 5) & 0xF,
											fno.fdate & 0x1F,
											(fno.ftime >> 11) & 0x1F,
